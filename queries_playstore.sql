...
SELECT analytics.genres, COUNT(*) FROM analytics GROUP BY analytics.genres;
 ...
 SELECT analytics.category,AVG(rating) AS average_rating FROM analytics GROUP BY analytics.category ORDER BY average_rating desc;

 --Find the name, price, and rating of the most expensive app with a rating that’s less than 3.--
 SELECT app_name,price,rating FROM analytics WHERE rating < 3 ORDER BY price desc LIMIT 1;

 --Find all apps with a min install not exceeding 50, that have a rating. Order your results by highest rated first.--
 SELECT * FROM analytics WHERE analytics.min_installs <=50 AND rating IS NOT NULL ORDER BY rating DESC;
...
SELECT app_name, reviews, analytics.min_installs, analytics.min_installs/reviews AS proportion 
FROM analytics WHERE reviews >100000 ORDER BY proportion DESC LIMIT 1;
